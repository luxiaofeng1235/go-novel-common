...
func (a *Adapter) initPolicy() error {
	...
	e, err := casbin.NewSyncedEnforcer(viper.GetString("casbin.modelFile"),
		viper.GetString("casbin.policyFile"))
...
func (a *Adapter) SavePolicy(model model.Model) (err error) {
	...
	for ptype, ast := range model["p"] {
		for _, rule := range ast.Policy {
			...
			err = global.DB.Model(models.SysCasbinRule{}).Create(&line).Error
			if err != nil {
				return err
			}
		}
...
func (a *Adapter) dropTable() (err error) {
	var casbin_rule models.SysCasbinRule
	global.DB.Exec(fmt.Sprintf("DROP TABLE %s", casbin_rule.TableName()))
	return
}

func (a *Adapter) createTable() (err error) {
	var casbin_rule models.SysCasbinRule
	global.DB.Exec(fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s (ptype VARCHAR(10), v0 VARCHAR(256), v1 VARCHAR(256), v2 VARCHAR(256), v3 VARCHAR(256), v4 VARCHAR(256), v5 VARCHAR(256))", casbin_rule.TableName()))
	return
}
...
func (a *Adapter) LoadPolicy(model model.Model) error {
	...
	global.DB.Model(models.SysCasbinRule{}).Find(&lines)
	if err := global.DB.Model(models.SysCasbinRule{}).Find(&lines).Error; err != nil {
		return err
	}
...
func rawDelete(a *Adapter, line models.SysCasbinRule) error {
	db := global.DB
	...
	err := db.Delete(&models.SysCasbinRule{}).Error
	return err
}
